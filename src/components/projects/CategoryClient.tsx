...
import { getProjectsByCategory } from "@/services/projectApi";
import { IconLoader } from "@tabler/icons-react";


export default function CategoryClient({ easyid }: { easyid: string }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setIsLoading(true);
      try {
        const res = await getProjectsByCategory(easyid, 1, 20);
